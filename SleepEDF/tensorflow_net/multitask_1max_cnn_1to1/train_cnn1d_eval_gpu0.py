import os
os.environ["CUDA_VISIBLE_DEVICES"]='0' # wilson: set visible GPU
import numpy as np
...
if (eeg_active):
    ...
    eeg_eval_gen = DataGenerator(os.path.abspath(FLAGS.eeg_eval_data), data_shape=[config.time_length, config.freq_length], shuffle = False)
...
if (eog_active):
    ...
    eog_eval_gen = DataGenerator(os.path.abspath(FLAGS.eog_eval_data), data_shape=[config.time_length, config.freq_length], shuffle = False)
    # load pretrained filterbank and do filtering first
...
if (emg_active):
    ...
    emg_eval_gen = DataGenerator(os.path.abspath(FLAGS.emg_eval_data), data_shape=[config.time_length, config.freq_length], shuffle = False)
    # load pretrained filterbank and do filtering first
