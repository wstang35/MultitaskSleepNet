...
class DataGenerator:
    def __init__(self, filelist, data_shape=np.array([29, 129]),shuffle=False, test_mode=False):
        ...
        self.Ncat = 5 # sorry for the hard code
        # read from mat file
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(len(data_index)):
            ...
            if(self.test_mode == True and data_index[i] == 0): # handle the terminal epochs
    ...
    def rest_batch(self, batch_size):
        ...
        for i in range(len(data_index)):
            ...
            batch_y1[i] = self.y[data_index[i] - 1]
            batch_label1[i] = self.label[data_index[i] - 1]
            if(self.test_mode == True and data_index[i] == self.data_size - 1): # handle the terminal epochs
