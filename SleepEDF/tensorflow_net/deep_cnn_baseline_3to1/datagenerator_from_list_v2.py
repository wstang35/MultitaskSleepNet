...
class DataGenerator:
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(len(data_index)):
            ...
            if(self.test_mode == True and data_index[i] == 0): # handle terminal epochs (padding)
    ...
    def rest_batch(self, batch_size):
        ...
        for i in range(len(data_index)):
            ...
            batch_y1[i] = self.y[data_index[i] - 1]
            batch_label1[i] = self.label[data_index[i] - 1]
            if(self.test_mode == True and data_index[i] == self.data_size - 1): # handle terminal epochs (padding)
