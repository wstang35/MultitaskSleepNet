...
class DataGenerator:
    ...
    def next_batch(self, batch_size):
        ...
        batch_x = np.ndarray([batch_size, self.data_shape[0]*3, self.data_shape[1]])
        ...
        for i in range(len(data_index)):
            ...
            # concatentate to make contextual input
            if(self.test_mode == True and data_index[i] == 0): # handle the terminal epochs (padding)
    ...
    # get the rest of the data if they are smaller than 1 batch
    ...
    def rest_batch(self, batch_size):
        ...
        batch_x = np.ndarray([actual_len, self.data_shape[0]*3, self.data_shape[1]])
        ...
        for i in range(len(data_index)):
            ...
            batch_label[i] = self.label[data_index[i]]
            # concatentate to make contextual input
            if(self.test_mode == True and data_index[i] == self.data_size - 1): # handle the terminal epochs (padding)
